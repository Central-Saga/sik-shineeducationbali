...
export function SesiKerjaForm({
  ...
  const [formData, setFormData] = React.useState<SesiKerjaFormData>(() => {
    return {
      ...
      nomor_sesi: initialData?.nomor_sesi || 1,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.nomor_sesi || formData.nomor_sesi < 1) {
      newErrors.nomor_sesi = "Nomor sesi wajib diisi dan minimal 1.";
    }
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="nomor_sesi">
                Nomor Sesi <span className="text-destructive">*</span>
              </Label>
              <Input
                id="nomor_sesi"
                type="number"
                min="1"
                max="255"
                value={formData.nomor_sesi}
                onChange={(e) => {
                  setFormData(prev => ({ ...prev, nomor_sesi: parseInt(e.target.value) || 1 }));
                  setErrors(prev => {
                    const newErrors = { ...prev };
                    delete newErrors.nomor_sesi;
                    return newErrors;
                  });
                }}
                aria-invalid={!!errors.nomor_sesi}
                className={errors.nomor_sesi ? "border-destructive" : ""}
              />
              {errors.nomor_sesi && (
                <p className="text-sm text-destructive">{errors.nomor_sesi}</p>
              )}
            </div>
